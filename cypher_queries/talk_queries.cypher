    ...
    MATCH (plainFlour :Ingredient {name: "Plain flour"})
    MATCH (salt:Ingredient {name: "Salt"})
    MATCH (unsaltedButter:Ingredient {name: "Unsalted butter"})
    MERGE (tart)-[:REQUIRES {quantity: 225, unit: "g"}]->(plainFlour)
    ...
    MATCH (ph :Author {name: "John Doe"})
    MATCH (tart :Recipe {name: "Shallot, Onion & Chive Tart (John Doe)"})
    MERGE (ph)-[:HAS_WRITTEN {published_in: "https://www.johndoe-recipes.com/post/puff-pastry", publication_type: "WEBSITE"}]->(tart);
